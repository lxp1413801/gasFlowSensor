...
#include <process.h>
#include "modbus.h"
uint8_t uartReceivedBuf[256];
uint16_t uartReceivedLen=0;
bool uartReceivedFlg=false;
...
uint16_t crc_sum(uint8_t *puchMsg, uint16_t usDataLen)
{
    uint16_t len;
...
uint8_t crc_verify( uint8_t* message, uint16_t n)
{
    ...
    memcpy((uint8_t*)(&expected),(message+n-2),2);
...
/*
uint16_t modbus_pool_slave(uint8_t* buf)
{
	uint8_t sendBuf[64];
